# 个人知识库

技术学习知识库，使用 Obsidian 管理。

## 知识体系

### 📱 [Android 知识](Android知识/)

Android 高级开发完整知识体系，涵盖 10 个模块、57 篇文档。

| 模块 | 核心内容 |
|------|---------|
| Android基础 | Activity、Fragment、Service、Handler、事件分发、View绘制 |
| Android进阶 | Compose、Jetpack 系列、插件化热修复、APT/KSP |
| 性能优化 | 启动、内存、卡顿、包体积、电量、网络 |
| 自定义View | 绘制流程、Canvas/Paint、动画、事件冲突 |
| 组件化 | 架构拆分、组件通信、资源隔离、生命周期 |
| 跨进程通信 | Binder、AIDL、Messenger、ContentProvider |
| Gradle与构建 | 生命周期/Task、Transform/ASM、Plugin、依赖管理 |
| 开源框架 | OkHttp/Retrofit、Glide/Coil、RxJava、ARouter |
| 架构设计 | MVC/MVP/MVVM/MVI、Clean Architecture、依赖注入 |
| SDK设计 | API设计、初始化策略、版本管理、混淆、降级 |

### 🤖 [Agent AI 学习](Agent%20AI学习/)

AI Agent 开发学习知识体系，涵盖 9 个模块、20 篇文档。

| 模块 | 核心内容 |
|------|---------|
| 基础概念 | AI Agent 概念、LLM 基础、Prompt Engineering、Harness Engineering |
| 框架与工具 | LangChain、LlamaIndex、AutoGen/CrewAI |
| RAG检索增强生成 | RAG 原理与架构、向量数据库、优化策略 |
| Agent设计模式 | ReAct、Plan-and-Execute、Multi-Agent 协作 |
| Function Calling | 原理与实践、自定义工具、MCP 协议 |
| 记忆与状态管理 | 短期/长期记忆、状态持久化 |
| 应用实战 | 知识库问答、代码助手、数据分析 Agent |
| 部署与优化 | 部署架构、性能优化、成本控制、监控 |
| 扩展知识 | 端侧推理、多模态 AI、评测体系 |

### 💻 [LeetCode 刷题](LeetCode刷题/)

面试高频算法题 119 道，Java 实现，含思路、代码、复杂度分析。

| 专题 | 题数 | 核心技巧 |
|------|------|---------|
| [数组与哈希](LeetCode刷题/1-数组与哈希.md) | 15 | HashMap、前缀和、原地哈希、桶排序 |
| [双指针与滑动窗口](LeetCode刷题/2-双指针与滑动窗口.md) | 12 | 左右指针、快慢指针、窗口模板 |
| [链表](LeetCode刷题/3-链表.md) | 10 | dummy 节点、快慢指针、递归反转 |
| [二叉树](LeetCode刷题/4-二叉树.md) | 15 | DFS/BFS、递归三要素、前中后序 |
| [回溯](LeetCode刷题/5-回溯.md) | 8 | 选择-递归-撤销、剪枝、去重 |
| [动态规划](LeetCode刷题/6-动态规划.md) | 20 | 状态转移、背包、区间 DP、空间优化 |
| [栈与队列](LeetCode刷题/7-栈与队列.md) | 8 | 单调栈、括号匹配、滑动窗口最大值 |
| [图与BFS/DFS](LeetCode刷题/8-图与BFS-DFS.md) | 7 | 拓扑排序、多源 BFS、连通分量 |
| [贪心](LeetCode刷题/9-贪心.md) | 5 | 区间调度、跳跃游戏、局部最优 |
| [堆与优先队列](LeetCode刷题/10-堆与优先队列.md) | 6 | TopK、合并K个、中位数 |
| [二分查找](LeetCode刷题/11-二分查找.md) | 6 | 左右边界、旋转数组、最小化最大值 |
| [并查集](LeetCode刷题/12-并查集.md) | 4 | 连通分量、路径压缩、合并集合 |
| [位运算](LeetCode刷题/13-位运算.md) | 6 | 异或、位计数、缺失数字 |
