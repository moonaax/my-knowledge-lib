...
## 八、人机协作节点（Human-in-the-loop）

> 🔑 在工具执行前暂停，等待用户确认或拒绝，是 LangGraph 的 `interrupt_before` 机制。

### 8.1 为什么需要人机协作

自纠错和 Plan-and-Execute 都是全自动的——Agent 决定调用什么工具就直接执行。但在某些场景下，用户需要在关键操作前确认：
...
| 写入操作 | 修改数据前需要确认 |
| 敏感查询 | 用户不想暴露某些信息 |
| 学习/演示 | 需要观察 Agent 的决策过程 |

### 8.2 interrupt_before 原理

LangGraph 编译图时，可以通过 `interrupt_before=["节点名"]` 在指定节点前暂停执行：
...
执行流程：
1. 调用 `invoke()` → 图执行到 `tools` 节点前暂停
2. 通过 `get_state(config)` 获取当前快照，查看 `next` 字段确认暂停位置
3. 用户确认 → `invoke(None, config=config)` 恢复执行
4. 用户拒绝 → `update_state(config, {"messages": rejection_msgs})` 注入拒绝消息后恢复
...
if snapshot.next and "tools" in snapshot.next:
    # 暂停在 tools 节点，提取待执行的工具调用
    last_msg = snapshot.values["messages"][-1]
    tool_calls = last_msg.tool_calls  # [{id, name, args}, ...]
    
    # 确认执行
    result = human_app.invoke(None, config=config)
    
    # 拒绝执行 — 注入 ToolMessage 告知 LLM 工具被拒绝
    rejection_msgs = [
        ToolMessage(content="用户拒绝了此工具调用。请直接用你的知识回答。", 
                    tool_call_id=tc["id"])
...
````
START → agent(LLM推理) ──有 tool_calls──→ [中断点] ──确认──→ tools → agent
                                              │
                                              └──拒绝──→ 注入拒绝消息 → agent
...
两个端点，非 SSE 协议（直接返回 JSON）：

| 端点 | 说明 |
|------|------|
| `POST /human_chat` | 发送消息，可能返回 `confirm_required` 或 `done` |
| `POST /human_confirm` | 确认/拒绝工具调用，返回最终回答 |

```python
# /human_chat 响应
{"status": "confirm_required", "tool_calls": [{"id": "...", "name": "search_weather", "args": {"city": "北京"}}]}
# 或
{"status": "done", "content": "你好！有什么可以帮你的吗？"}

# /human_confirm 请求
{"session_id": "xxx", "confirm": true}   # 批准
{"session_id": "xxx", "confirm": false}  # 拒绝
```
...
| 组件 | 说明 |
|------|------|
| `ToolConfirmCard` | 黄色毛玻璃卡片，显示待确认的工具调用列表，带"批准"和"拒绝"按钮 |
| `confirm_required` ContentBlock | 新增的块类型，存储 toolCalls 数组 |
| 非 SSE 模式 | human 模式直接返回 JSON，不走 SSE 流 |
| 模式切换 | 四档循环：⚡ Agent → 🔗 LangGraph → 📋 Plan → 🤝 Human |

### 8.7 与其他模式的对比

| 模式 | 工具执行 | 控制粒度 | 适用场景 |
|------|---------|---------|---------|
| AgentExecutor | 全自动 | 无 | 简单问答 |
| LangGraph + 自纠错 | 全自动（失败重试） | 中 | 一般任务 |
| Plan-and-Execute | 全自动（先规划） | 中高 | 复杂多步骤 |
| **人机协作** | **暂停确认** | **最高** | 敏感操作 / 学习演示 |
...
第四阶段架构（全链路）：
  后端（四种模式共存）：
    ...
    人机协作模式:        StateGraph(agent → [interrupt] → tools) → JSON 响应
    四种模式共享 tools.py 工具定义

  前端：
    SSE 事件: tool_start / tool_end / tool_retry / plan_start / plan_step / token / done
    JSON 响应: confirm_required / done（人机协作模式专用）
    UI 组件: ToolCard / RetryCard / PlanCard / PlanStepCard / ToolConfirmCard
    模式切换: ⚡ Agent ↔ 🔗 LangGraph + 自纠错 ↔ 📋 Plan-and-Execute ↔ 🤝 人机协作
````
