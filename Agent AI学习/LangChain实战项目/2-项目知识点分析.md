# 项目知识点分析

> 持续更新，记录每个阶段用到的知识点和对应的学习文档。

## 第一阶段：多轮对话客户端

### 1. 终端版 (`chat.py`)

| 代码                                    | 知识点                                     | 对应文档                            |
| ------------------------------------- | --------------------------------------- | ------------------------------- |
| `ChatOpenAI(base_url=..., model=...)` | Model I/O — 模型初始化，DeepSeek 兼容 OpenAI 接口 | [[../LangChain专题/2-Model IO详解]] |
| `ChatPromptTemplate.from_messages()`  | Prompt Template — 系统提示 + 用户输入模板         | [[../LangChain专题/2-Model IO详解]] |
| `MessagesPlaceholder("history")`      | 动态消息占位符 — 运行时插入历史对话                     | [[../LangChain专题/2-Model IO详解]] |
| `prompt \| llm`                       | LCEL 管道符 — 把 prompt 和 llm 串成 Chain      | [[../LangChain专题/3-LCEL表达式语言]]  |
| `RunnableWithMessageHistory`          | Memory 记忆管理 — 自动存取对话历史                  | [[../LangChain专题/2-Model IO详解]] |
| `InMemoryChatMessageHistory`          | 会话存储 — 内存中保存消息列表                        | [[../LangChain专题/2-Model IO详解]] |
| `chain.stream(...)`                   | 流式输出 — 逐 chunk 返回                       | [[../LangChain专题/3-LCEL表达式语言]]  |
| `python-dotenv` + `.env`              | 环境变量管理 — API Key 不硬编码                   | 工程实践                            |

### 2. FastAPI 后端 (`server.py`)

| 代码 | 知识点 | 对应文档 |
|------|--------|---------|
| `FastAPI()` + `@app.post("/chat")` | REST API — 把 Chain 暴露为 HTTP 接口 | [[../LangChain专题/7-生产化部署]] |
| `StreamingResponse` + SSE | Server-Sent Events — `data: xxx\n\n` 推送流式数据 | [[../LangChain专题/7-生产化部署]] |
| `chain.astream(...)` | 异步流式调用 — `async for` 配合 FastAPI | [[../LangChain专题/3-LCEL表达式语言]] |
| `CORSMiddleware` | 跨域处理 — Electron 请求 localhost | Web 工程基础 |
| Pydantic `BaseModel` | 请求体校验 — 自动验证 JSON 参数 | FastAPI 基础 |

### 3. Electron 桌面客户端 (`electron/`)

| 文件 | 知识点 | 说明 |
|------|--------|------|
| `main.js` — `BrowserWindow` | Electron 主进程 — 创建窗口、加载 HTML | Electron 基础 |
| `titleBarStyle: 'hiddenInset'` | macOS 原生窗口 — 隐藏标题栏保留红绿灯 | Electron macOS 适配 |
| `package.json` | Node.js 项目管理 — 依赖和启动脚本 | Node.js 基础 |

### 4. 前端界面 (`index.html`)

| 功能 | 知识点 |
|------|--------|
| `fetch()` + `ReadableStream` | Fetch API 流式读取 — 逐块解析 SSE |
| `data-theme` + CSS 变量 | 主题系统 — 深浅色切换，`localStorage` 持久化 |
| `backdrop-filter: blur()` | 毛玻璃效果 |
| `radial-gradient` + `animation` | 环境光晕 — 渐变球体漂移动画 |
| `mask` + `linear-gradient` | 渐变发光边框 — mask-composite 技巧 |
| `@keyframes msgIn` | 消息入场动画 — 淡入 + 上移 |
| `compositionstart/end` | 输入法兼容 — 防止 IME 回车误触发 |
| `text-overflow: ellipsis` | 文本溢出省略 |
| `-webkit-app-region: drag` | Electron 窗口拖拽区域 |

### 5. 工程化

| 操作 | 知识点 |
|------|--------|
| `start.sh` | Shell 脚本 — 后台启动后端 + 前台启动 Electron |
| `.env` / `.env.example` | 密钥管理 — 模板 + gitignore |
| Git 提交管理 | 版本控制 — feat/fix/docs 提交规范 |

---

## 知识覆盖度

````
                        已覆盖    未覆盖
LangChain 专题
├── 1-学习路线           ✅ 已读
├── 2-Model IO          ████████  ChatModel、Prompt、Memory
├── 3-LCEL              ██████░░  管道、流式、astream_events（并行/分支/重试待学）
├── 4-Tool与Agent       ██████░░  @tool、create_tool_calling_agent、AgentExecutor（Callback自定义/LangSmith待学）
├── 5-RAG实战           ██████░░  Loader、Splitter、Embedding、FAISS、Retriever（MultiQuery/Compression/Ensemble待学）
├── 6-LangGraph编排     ░░░░░░░░  第四阶段
└── 7-生产化部署         ██░░░░░░  FastAPI+SSE（LangSmith/缓存/安全待学）
````
---

## 第二阶段：工具调用 Agent

> 日期：2026-04-21 | 代码：`~/lib/langchain-deepseek-chat/`

### 1. 工具定义 (`chat.py` / `server.py`)

| 代码              | 知识点                                               | 对应文档                            |
| --------------- | ------------------------------------------------- | ------------------------------- |
| `@tool` 装饰器     | Tool — 最简工具定义方式，自动提取 name/description/args_schema | [[../LangChain专题/4-Tool与Agent]] |
| `calculator` 工具 | 安全沙箱 — `eval` + `{"__builtins__": {}}` 限制执行环境     | [[../LangChain专题/4-Tool与Agent]] |
| 工具 docstring    | 工具描述 — LLM 完全依赖描述决定何时使用，质量直接影响 Agent 表现           | [[../LangChain专题/4-Tool与Agent]] |
| `tools = [...]` | 工具列表 — 控制在 3-8 个，太多会降低选择准确率                       | [[../LangChain专题/4-Tool与Agent]] |

### 2. Agent 构建 (`chat.py` / `server.py`)

| 代码                                              | 知识点                                              | 对应文档                            |
| ----------------------------------------------- | ------------------------------------------------ | ------------------------------- |
| `MessagesPlaceholder("agent_scratchpad")`       | Agent Prompt — 与普通 Chain 的关键区别，存放中间推理过程          | [[../LangChain专题/4-Tool与Agent]] |
| `create_tool_calling_agent(llm, tools, prompt)` | Function Calling Agent — 基于 LLM 原生能力，比 ReAct 更准确 | [[../LangChain专题/4-Tool与Agent]] |
| `AgentExecutor(agent, tools, ...)`              | 推理循环引擎 — 执行 Thought→Action→Observation 循环        | [[../LangChain专题/4-Tool与Agent]] |
| `max_iterations=10`                             | 安全边界 — 防止 Agent 无限循环                             | [[../LangChain专题/4-Tool与Agent]] |
| `handle_parsing_errors=True`                    | 错误恢复 — 自动处理 LLM 输出格式错误                           | [[../LangChain专题/4-Tool与Agent]] |
| `RunnableWithMessageHistory` 包装 AgentExecutor   | Memory + Agent — 记忆机制同样适用于 Agent                 | [[../LangChain专题/2-Model IO详解]] |

### 3. Agent API 流式推送 (`server.py`)

| 代码 | 知识点 | 对应文档 |
|------|--------|---------|
| `astream_events(version="v2")` | 细粒度事件流 — 比 astream 更详细，能捕获工具调用过程 | [[../LangChain专题/3-LCEL表达式语言]] |
| `on_tool_start` / `on_tool_end` | 工具事件 — 获取工具名、输入参数、返回结果 | [[../LangChain专题/4-Tool与Agent]] |
| `on_chat_model_stream` | LLM token 事件 — 最终回答的逐字输出 | [[../LangChain专题/3-LCEL表达式语言]] |
| `event: tool_start\ndata: {...}\n\n` | SSE 自定义事件类型 — 标准 SSE 支持 event 字段区分消息类型 | [[../LangChain专题/7-生产化部署]] |

### 4. 前端工具调用展示 (`index.html`)

| 功能 | 知识点 |
|------|--------|
| SSE 多事件类型解析 | 按 `\n\n` 分割消息，解析 `event:` 和 `data:` 字段 |
| `.tool-call` 卡片 | 工具调用可视化 — pending/done 状态切换 |
| 文本节点管理 | DOM 操作 — 工具卡片后追加文本需要新建 TextNode |
| `escHtml()` | XSS 防护 — 转义工具返回的 HTML 特殊字符 |

### 5. 架构变化

````
第一阶段: prompt | llm → RunnableWithMessageHistory → stream/astream
第二阶段: create_tool_calling_agent → AgentExecutor → RunnableWithMessageHistory → astream_events
第三阶段: 同上，新增 knowledge_search 工具（内部: FAISS Retriever → 格式化文档 → 返回给 Agent）
````

---

## 第三阶段：知识库 RAG 问答

> 日期：2026-04-21 | 代码：`~/lib/langchain-deepseek-chat/`

### 1. 离线索引构建 (`build_index.py`)

| 代码 | 知识点 | 对应文档 |
|------|--------|---------|
| `DirectoryLoader(glob="**/*.md")` | Document Loader — 递归扫描目录，按 glob 匹配文件批量加载 | [[../LangChain专题/5-RAG实战]] |
| `TextLoader(encoding="utf-8")` | 文本加载器 — 以纯文本方式加载 Markdown，保留原始格式 | [[../LangChain专题/5-RAG实战]] |
| `RecursiveCharacterTextSplitter(chunk_size=500, chunk_overlap=50)` | 文本分块 — 按分隔符优先级递归分割，中文优化 separators | [[../LangChain专题/5-RAG实战]] |
| `separators=["\n\n", "\n", "。", "！", "？", ...]` | 中文分隔符 — 优先段落→行→句子→字符，保持语义完整 | [[../LangChain专题/5-RAG实战]] |
| `HuggingFaceEmbeddings(model_name="BAAI/bge-base-zh-v1.5")` | Embedding 模型 — 本地中文向量化，768 维，无需 API Key | [[../LangChain专题/5-RAG实战]] |
| `normalize_embeddings=True` | 向量归一化 — 使余弦相似度等价于内积，提高检索效率 | [[../LangChain专题/5-RAG实战]] |
| `FAISS.from_documents(chunks, embeddings)` | 向量存储 — 一步完成向量化 + FAISS 索引构建 | [[../LangChain专题/5-RAG实战]] |
| `vectorstore.save_local("faiss_index")` | 索引持久化 — 序列化到磁盘（index.faiss + index.pkl） | [[../LangChain专题/5-RAG实战]] |

使用方式：

````bash
# 构建索引（首次需下载 Embedding 模型约 400MB）
python3 build_index.py                      # 默认加载 ~/lib/my-knowledge-lib
python3 build_index.py /path/to/docs        # 指定文档目录

# 国内网络用镜像加速下载模型
HF_ENDPOINT=https://hf-mirror.com python3 build_index.py
````

### 2. RAG 工具封装 (`tools.py`)

| 代码 | 知识点 | 对应文档 |
|------|--------|---------|
| `FAISS.load_local(path, embeddings, allow_dangerous_deserialization=True)` | 加载索引 — 从磁盘反序列化，需传入相同的 Embedding 模型 | [[../LangChain专题/5-RAG实战]] |
| `vectorstore.as_retriever(search_kwargs={"k": 3})` | Retriever — 从 VectorStore 创建检索器，返回 top-k 相似文档 | [[../LangChain专题/5-RAG实战]] |
| `retriever.invoke(query)` | 检索调用 — 输入查询文本，返回 Document 列表 | [[../LangChain专题/5-RAG实战]] |
| 懒加载模式 `_get_retriever()` | 性能优化 — 首次调用时才加载索引，避免启动时阻塞 | 工程实践 |
| `@tool knowledge_search` | RAG + Agent 结合 — 将检索封装为工具，Agent 自主决定是否使用 | [[../LangChain专题/4-Tool与Agent]] [[../LangChain专题/5-RAG实战]] |

使用方式：

````python
# tools.py 导出 all_tools 列表，chat.py 和 server.py 统一导入
from tools import all_tools

# Agent 会根据用户问题自动判断是否调用 knowledge_search：
#   "LCEL 是什么？"        → 调用 knowledge_search("LCEL")
#   "今天几号？"            → 调用 get_current_time()
#   "你好"                 → 直接回答，不调用工具
````

### 3. 工具模块化重构

| 改动 | 知识点 |
|------|--------|
| 新建 `tools.py` 集中管理工具 | 模块化 — 消除 chat.py 和 server.py 中的重复工具定义 |
| `from tools import all_tools` | 单一数据源 — 两端共享同一份工具列表，新增工具只改一处 |
| Prompt 新增 knowledge_search 说明 | 工具引导 — 告诉 Agent "技术问题优先从知识库检索" |

### 4. RAG 架构说明

````
离线阶段（build_index.py 一次性执行）：
  Markdown 文件 → DirectoryLoader → TextSplitter → HuggingFace Embedding → FAISS 持久化

在线阶段（Agent 运行时按需调用）：
  用户提问 → Agent 判断需要检索 → knowledge_search 工具
    → FAISS Retriever 检索 top-3 → 格式化文档+来源 → 返回给 Agent
    → Agent 基于检索结果生成回答
````

### 5. 踩坑记录

1. **HuggingFace 下载超时**：国内网络无法直连 huggingface.co，用 `HF_ENDPOINT=https://hf-mirror.com` 环境变量走镜像
2. **HuggingFaceEmbeddings 弃用警告**：`langchain_community` 中的类将迁移到 `langchain-huggingface` 包，暂不影响功能
3. **运行时仍尝试联网**：模型下载完后，`sentence-transformers` 每次加载仍会尝试连 HuggingFace 检查更新，导致卡住。解决：启动时加 `HF_HUB_OFFLINE=1` 强制离线模式
4. **ClearRequest 接口报错**：第二阶段给 `ChatRequest` 加了 `message` 必填字段后，`/clear` 接口复用 `ChatRequest` 导致 422 错误。解决：单独定义 `ClearRequest(session_id)` 模型
5. **astream_events 推送中间推理**：Agent 多次调用 LLM（决策+最终回答），`on_chat_model_stream` 会捕获所有 token 包括中间推理（"我来帮您搜索..."）。解决：用 `in_tool` 标志位，工具调用期间暂停推送 token
6. **Markdown 渲染未完成**：DeepSeek 返回的 token 流中 `###` 标题前没有换行符，`marked.js` 无法识别。尝试了 CDN/npm/本地 UMD/流式渲染等方案，均有兼容性问题。暂时保持纯文本输出，后续优化

### 6. 待优化

- [ ] 前端 Markdown 渲染（需处理 DeepSeek 非标准格式）
- [ ] 工具调用中间推理过程的展示优化（目前直接过滤掉了）

---

## 当前项目能力盘点（截至第三阶段）

````
已具备                              缺失（面试有区分度的能力）
─────────────────────────────────────────────────────────────
✅ LLM 接入（DeepSeek）              ❌ 多步推理（拆解复杂任务）
✅ 多轮对话（InMemory）              ❌ 自纠错（失败自动重试换策略）
✅ 工具调用（@tool + AgentExecutor）  ❌ 规划能力（先计划再执行）
✅ RAG 检索（FAISS + bge-zh）        ❌ 多 Agent 协作
✅ 流式输出（SSE + astream_events）  ❌ 记忆持久化（重启就忘）
✅ Electron 桌面客户端               ❌ RAG 效果评估（无量化指标）
                                    ❌ 可观测性（无链路追踪）
                                    ❌ 生产化部署（无 Docker）
````

**现状定位**：能调工具的聊天机器人，还不是能自主完成复杂任务的智能体。

---

## 下一步规划

### 第四阶段：LangGraph 多步骤 Agent（优先级最高 ⭐⭐⭐）

> 从 AgentExecutor 升级到 LangGraph，是"聊天机器人"到"真正 Agent"的关键跳跃
> 对应文档：[[../LangChain专题/6-LangGraph编排]]

**目标**：用 StateGraph 重构现有 Agent，实现多步推理和自纠错

| 要做的事 | 涉及知识点 | 说明 |
|---------|----------|------|
| 用 StateGraph 替换 AgentExecutor | State、Node、Edge、Graph | 从黑盒变成可控的状态图 |
| 实现条件路由 | conditional_edge | Agent 根据结果决定下一步 |
| 实现自纠错循环 | 循环边、重试逻辑 | 工具调用失败时自动换策略 |
| 添加人机协作节点 | interrupt_before | 关键操作前暂停等人确认 |
| 状态持久化 | MemorySaver / SqliteSaver | 断点恢复，不怕进程重启 |

**预期架构**：

````
START → route（判断意图）
         ├→ direct_answer（直接回答）→ END
         ├→ tool_call（调用工具）→ check_result → 成功 → END
         │                                     → 失败 → retry/换策略
         └→ rag_search（知识库检索）→ generate_answer → END
````

### 第 4.5 阶段：记忆持久化 + RAG 优化（优先级中 ⭐⭐）

| 要做的事 | 涉及知识点 | 说明 |
|---------|----------|------|
| 记忆换成 SQLite | SQLChatMessageHistory | 重启不丢失，改动很小 |
| 混合检索 | EnsembleRetriever（向量+BM25） | 提高检索召回率 |
| 构建评测集 | 手动标注 20-30 个 QA 对 | 量化 RAG 准确率，面试能说数据 |
| 前端 Markdown 渲染 | marked.js + 预处理 | 解决 DeepSeek 非标准格式问题 |

### 第五阶段：生产化部署（优先级中 ⭐⭐）

> 对应文档：[[../LangChain专题/7-生产化部署]]

| 要做的事 | 涉及知识点 | 说明 |
|---------|----------|------|
| Docker 容器化 | Dockerfile + docker-compose | 一键部署 |
| LangSmith 链路追踪 | LANGCHAIN_TRACING_V2 | 可观测性，调试利器 |
| 成本统计 | Callback + token 计数 | 面试能说成本控制方案 |
| API 认证 | FastAPI 中间件 | 安全基础 |

### 简历呈现建议

项目名改为有业务场景的：**"基于 LangGraph 的技术知识库智能问答系统"**

````
技术栈：LangChain + LangGraph + DeepSeek + FAISS + FastAPI + Electron
亮点：
1. LangGraph 状态图编排，支持多步推理、自纠错、条件路由
2. RAG 混合检索（向量+BM25），准确率 XX%（有评测数据）
3. 4 个自定义工具，Agent 自主决策调用
4. SSE 流式输出 + astream_events 细粒度事件推送
5. SQLite 持久化记忆，支持多会话隔离
6. LangSmith 全链路追踪，Token 成本统计
````

---

## 第四阶段：LangGraph 多步骤 Agent

> 🔲 待开始

---

## 第五阶段：生产化部署

> 🔲 待开始
