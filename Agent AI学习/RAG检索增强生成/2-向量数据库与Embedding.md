...
> **本文定位：** Embedding 和向量数据库是 RAG 的两大基础设施——Embedding 把文本变成向量，向量数据库负责高效存储和检索。本文覆盖模型选型、数据库对比、相似度计算、索引优化的完整知识。
## 1. Embedding 基础
...
> **选对 Embedding 模型是 RAG 成功的一半：** 模型决定了语义理解的质量。中文场景优先选 BGE 系列（本地部署、中文优化），英文场景用 OpenAI text-embedding-3。
## 2. Embedding 模型选型
...
> **两种主流方案：** API 调用（OpenAI，简单但有成本）和本地部署（BGE，免费且中文效果好）。生产环境中文场景推荐 bge-large-zh。
````python
# OpenAI Embedding
...
> **关键代码解读：**
> - OpenAI Embedding 通过 API 调用，简单但有成本和隐私顾虑
> - BGE 模型可以本地部署，中文效果优秀，`query_instruction` 是查询时的前缀指令
> - `embed_query()` 和 `embed_documents()` 的区别：query 会加前缀指令，documents 不加
````

> **向量数据库是 RAG 的存储引擎：** Embedding 模型把文本转成向量，向量数据库负责高效存储和检索这些向量。选型的关键是数据量和部署环境。
## 3. 向量数据库
...
> **Chroma 是开发阶段的最佳选择：** 零配置、内嵌运行、API 简洁。但数据量超过百万级时需要换成 Milvus 或 Pinecone。
````python
import chromadb
...
> **关键代码解读（Chroma）：**
> - 内存模式适合开发测试，持久化模式数据写入磁盘
> - `from_documents()` — 一步完成 Embedding + 存储
> - `persist_directory` — 指定持久化路径，下次直接加载不用重新 Embedding
> - 支持增量添加（`add_documents`）和删除（`delete`），适合文档动态更新
````
### 3.3 FAISS（高性能检索）
...
> **关键代码解读（FAISS）：**
> - FAISS 是 Meta 开源的高性能向量检索库，纯 C++ 实现，速度极快
> - `save_local` / `load_local` — 索引持久化到本地文件
> - `merge_from` — 合并多个索引，适合增量构建
> - FAISS 不支持元数据过滤，需要配合外部存储使用
````
### 3.4 Milvus（生产环境）
...
> **关键代码解读（Milvus）：**
> - `connections.connect()` — 连接 Milvus 服务（需要单独部署）
> - `expr` 参数支持元数据过滤，可以在向量检索的同时按条件筛选
> - Milvus 支持分布式部署、亿级数据、多租户隔离，是生产环境首选
````

> **相似度计算是向量检索的数学基础：** 把文本转成向量后，"语义相似"就变成了"向量距离近"。不同的距离度量适合不同场景。
## 4. 相似度计算
...
> **关键代码解读：** 三种距离度量的 numpy 实现。余弦相似度衡量方向相似性（不受向量长度影响），欧氏距离衡量绝对距离，内积在向量归一化后等价于余弦相似度。
````
### 4.2 选择建议
...
> **为什么需要索引优化：** 暴力搜索（逐一比较所有向量）在数据量大时太慢。ANN（近似最近邻）索引通过牺牲少量精度换取数量级的速度提升。
## 5. 索引优化
...
> **思路：** 不同数据量选不同索引。小数据量用 Flat（精确搜索），中等用 IVF（聚类加速），大数据量用 HNSW（图搜索，速度和精度的最佳平衡）。
````python
import faiss
...
> **关键代码解读（FAISS 索引选择）：**
> - `IndexFlatL2` — 暴力搜索，精确但慢，适合 <10 万条数据
> - `IndexIVFFlat` — 先聚类再搜索，`nprobe` 控制搜索的聚类数量，越大越精确但越慢
> - `IndexHNSWFlat` — 基于图的近似搜索，`M=32` 是每个节点的连接数，越大越精确但内存越大
````

> **实践建议是踩坑总结：** Embedding 质量和检索效果是 RAG 系统的生命线。下面的清单覆盖了最常见的优化手段。
## 6. 实践建议
