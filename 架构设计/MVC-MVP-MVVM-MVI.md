...
> 关联主题：[[Jetpack-Lifecycle与ViewModel]] · [[Android-DataBinding]] · [[Kotlin-协程与Flow]] · [[Jetpack-Compose]]
...
- ViewModel：持有 UI 状态（[[Jetpack-Lifecycle与ViewModel]]），不持有 View 引用
...
**优点：** ViewModel 自动感知生命周期（[[Jetpack-Lifecycle与ViewModel]]），配置变更时数据不丢失，无需手动 detach。
...
3. 结合 [[Jetpack-Lifecycle与ViewModel]] 的 `LifecycleObserver`，让 Presenter 感知生命周期自动解绑
